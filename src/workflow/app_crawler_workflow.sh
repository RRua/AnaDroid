...
pullTestResultsFromDevice(){
	...
	has_output_directory=$(grep -o "The output directory is.*" crawlerTestOutput.log )
	if [[ -n "$has_output_directory" ]]; then
		outp_dir=$(echo "$has_output_directory" | sed 's/The output directory is //g' )
		if [[ -d "$outp_dir" ]]; then # sanity check
			debug_echo "copying crawler outdir "	
			#remove modified apks generated by app crawler
			find "$outp_dir" -name "*.apk" | xargs rm 
			debug_echo "moving output dir"
			mv "$outp_dir" "$localDir"
		fi
	fi
