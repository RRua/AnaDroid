...
sep_criteria="app_category_major" #"major" #"minor # categories

min_csv_row_len=1

test_type="monkey" # "monkey"
sep_by_type=True #False
...
class Statistics(object):
    ...
    def printStats(self, metric_name):
        print( "%s increases overall %f" %( metric_name , safe_division( self.total_value_increases ,  ( self.total_value_increases + self.total_value_decreases +self.total_value_equal ) ) ) )
        print( "%s decreases overall %f" %( metric_name , safe_division( self.total_value_decreases ,  ( self.total_value_increases + self.total_value_decreases + self.total_value_equal  ) ) ) )
        
        print( "%s increases major %f" %( metric_name , safe_division(self.total_value_increases_major ,  ( self.total_value_increases_major + self.total_value_decreases_major  + self.total_value_equal_major  ) ) ) )
        print( "%s decreases major %f" %( metric_name , safe_division(self.total_value_decreases_major ,  ( self.total_value_increases_major + self.total_value_decreases_major + self.total_value_equal_major ) ) ) )
        
        print( "%s increases major_wide %f" %( metric_name , safe_division(self.total_value_increases_major_wide ,  ( self.total_value_increases_major_wide + self.total_value_decreases_major_wide+ self.total_value_equal_major_wide  ) ) ) ) 
        print( "%s decreases major_wide %f" %( metric_name , safe_division(self.total_value_decreases_major_wide ,  ( self.total_value_increases_major_wide + self.total_value_decreases_major_wide + self.total_value_equal_major_wide ) ) ) ) 
        
        print( "%s increases minors %f" %(metric_name ,  safe_division(self.total_value_increases_minor ,  ( self.total_value_increases_minor + self.total_value_decreases_minor + self.total_value_equal_minor ) ) ) )
        print( "%s decreases minors %f" %(metric_name ,  safe_division(self.total_value_decreases_minor ,  ( self.total_value_increases_minor + self.total_value_decreases_minor + self.total_value_equal_minor ) ) ) )
        
        print( "%s increases minor wide %f" %( metric_name , safe_division(self.total_value_increases_minor_wide ,  ( self.total_value_increases_minor_wide + self.total_value_decreases_minor_wide+ self.total_value_equal_minor_wide  ) ) ) )
        print( "%s decreases minor wide %f" %( metric_name , safe_division(self.total_value_decreases_minor_wide ,  ( self.total_value_increases_minor_wide + self.total_value_decreases_minor_wide+ self.total_value_equal_minor_wide  ) ) ) )
        
        print( "%s increases patch %f" %( metric_name , safe_division(self.total_value_increases_patch ,  ( self.total_value_increases_patch + self.total_value_decreases_patch+ self.total_value_equal_patch  ) ) ) )
        print( "%s decreases patch %f" %( metric_name , safe_division(self.total_value_decreases_patch ,  ( self.total_value_increases_patch + self.total_value_decreases_patch+ self.total_value_equal_patch ) ) ) )
        
        print( "%s increases patch_wide %f" %( metric_name , safe_division(self.total_value_increases_patch_wide ,  ( self.total_value_increases_patch_wide + self.total_value_decreases_patch_wide + self.total_value_equal_patch_wide  ) ) ) ) 
        print( "%s decreases patch_wide %f" %( metric_name , safe_division(self.total_value_decreases_patch_wide ,  ( self.total_value_increases_patch_wide + self.total_value_decreases_patch_wide + self.total_value_equal_patch_wide  ) ) ) ) 
...
def generate_box_plot_from_metric(csvs_dict, m_type , title, xlabel, ylabel, x, y, z=None, agg_criteria=None, filter_zeros=True, min_samples=3, min_apps=3):
    ...
    for app_id ,v in csvs_dict.items():
        ...
        for av,zz in v.items():
            i = i+1
            criteria=(str(getCriteria(av, agg_criteria,app_category=(zz[1][0][-1] if len(zz)>0 else 'unknown'))) + "_" + zz[4]) if sep_by_type else str(getCriteria(av, agg_criteria,app_category=(zz[1][0][-1] if len(zz)>0 else 'unknown'))) 
    ...
    plt.setp(xtickNames, rotation=90, fontsize=3)
...
def generate_app_versions_dict(all_csvs_of_folder):
    ...
    for csv_file in all_csvs_of_folder:
        ...
        if len( sortedlist) >= min_csv_row_len :
            ...
            #app_id = getCriteria(app_id, agg_criteria=app_category )
    ...
    for v in csvs_dict:
        ...
        app_list_pairs_version_energy = list( map( lambda zz : (zz[0],zz[1][-3][1]) , csvs_dict[v].items()))
...
def getCriteria(app_version, agg_criteria=None, app_category=None ):
    ret_criteria = ""
    ...
    if "major" in agg_criteria:
        ret_criteria = str(app_version.major) + "_" + ret_criteria
    if "minor" in agg_criteria:
        ret_criteria =  str(app_version.major) +"." + str(app_version.minor) + "_" + ret_criteria
    if ret_criteria=="":
        return agg_criteria
    return ret_criteria
...
if __name__== "__main__":
    if len(sys.argv) > 1:
        ...
        buildBoxPlots(csvs_dict, criteria=sep_criteria, filter_zeros=True, min_samples=5, min_apps=3)
        #plt.show()
    else:
        print ("bad arg len")